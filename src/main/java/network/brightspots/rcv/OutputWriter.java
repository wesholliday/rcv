...
class OutputWriter {
  ...
  // Condorcet pairwise results (null if not a Condorcet election)
  ...
  OutputWriter setCondorcetResult(CondorcetTabulator.CondorcetResult condorcetResult) {
    this.condorcetResult = condorcetResult;
    return this;
  }
  ...
  private void addContestSummaryRows(CSVPrinter csvPrinter, RoundTally round1Tally)
      throws IOException {
    addContestSummaryRows(csvPrinter, round1Tally, false);
  }

  private void addContestSummaryRows(CSVPrinter csvPrinter, RoundTally round1Tally,
      boolean isCondorcet) throws IOException {
    BigDecimal numNoRankings =
    ...
    int numCandidates = isCondorcet && condorcetResult != null
        ? condorcetResult.candidates().size()
        : config.getNumCandidates();
    csvPrinter.printRecord("Contest Summary");
    csvPrinter.printRecord("Number to be Elected", numToBeElected);
    csvPrinter.printRecord("Number of Candidates", numCandidates);
    csvPrinter.printRecord("Total Number of Ballots", totalNumberBallots);
    String noRankingsLabel = isCondorcet
        ? "Number of Ballots with No Rankings"
        : "Number of Undervotes (No Rankings)";
    csvPrinter.printRecord(noRankingsLabel, numNoRankings);
    csvPrinter.println();
  }
  ...
  private void addContestInformationRows(CSVPrinter csvPrinter,
    ...
    addContestInformationRows(csvPrinter, winningThreshold, slice, sliceId, null);
  }
  ...
  private void addContestInformationRows(CSVPrinter csvPrinter,
                                         ...
                                         ContestConfig.TabulateBySlice slice,
                                         String sliceId,
                                         Footnote winnerTiebreakFootnote) throws IOException {
    ...
    if (config.isCondorcetEnabled()) {
      electionType = "Single-Winner (Condorcet)";
    } else if (config.isSingleWinnerEnabled()) {
      electionType = "Single-Winner";
    } else {
      electionType = "Multi-Winner";
    }
    csvPrinter.printRecord("Type of Election", electionType);
    ...
    for (int round = 1; round <= numRounds; round++) {
      for (Tabulator.TallyDecision decision : getCandidatesByDecisionType(
          round, Tabulator.TallyDecision.DecisionType.ELECTED)) {
        String winner = config.getNameForCandidate(decision.candidateName());
        if (winnerTiebreakFootnote != null && decision.wasDecidedViaTieBreak()) {
          winner += winnerTiebreakFootnote.getSymbol();
        }
        winners.add(winner);
      }
    }
    ...
    } else if (winningThreshold != null) {
      // Only non-slices print threshold information (not applicable for Condorcet)
  ...
  /** Resolved data for a single head-to-head pair, with winner listed first. */
  private record PairResult(
      String firstCandidate,
      String secondCandidate,
      String pairWinner,
      int margin,
      int firstAboveSecond,
      int secondAboveFirst,
      int tied,
      int neitherRanked,
      int oneRankedNoPreference) {}

  /** Wins/losses/ties summary for a single candidate. */
  private record CandidateRecord(String candidate, int wins, int losses, int ties) {}

  /** A candidate in the "best record" list with their smallest loss margin. */
  private record BestRecordEntry(String candidate, int smallestLossMargin) {}

  /**
   * Resolve all pairwise matchups from the CondorcetResult into PairResult records.
   * The winner of each matchup is listed first; ties use alphabetical order.
   */
  private List<PairResult> resolvePairResults() {
    List<String> candidates = condorcetResult.candidates();
    int[][] pref = condorcetResult.pref();
    int[][] tied = condorcetResult.tied();
    int[][] oneRankedNoPreference = condorcetResult.oneRankedNoPreference();
    int[][] neitherRanked = condorcetResult.neitherRanked();
    int[][] margins = condorcetResult.margins();
    int n = candidates.size();

    List<PairResult> pairs = new ArrayList<>();
    for (int i = 0; i < n; i++) {
      for (int j = i + 1; j < n; j++) {
        if (pref[j][i] > pref[i][j]) {
          pairs.add(new PairResult(
              candidates.get(j), candidates.get(i), candidates.get(j),
              margins[j][i], pref[j][i], pref[i][j],
              tied[i][j], neitherRanked[i][j], oneRankedNoPreference[i][j]));
        } else if (pref[i][j] > pref[j][i]) {
          pairs.add(new PairResult(
              candidates.get(i), candidates.get(j), candidates.get(i),
              margins[i][j], pref[i][j], pref[j][i],
              tied[i][j], neitherRanked[i][j], oneRankedNoPreference[i][j]));
        } else {
          pairs.add(new PairResult(
              candidates.get(i), candidates.get(j), "Tied",
              0, pref[i][j], pref[j][i],
              tied[i][j], neitherRanked[i][j], oneRankedNoPreference[i][j]));
        }
      }
  ...
  /** Compute strict wins/losses/ties for each candidate from the margins matrix. */
  private List<CandidateRecord> computeCandidateRecords() {
    List<String> candidates = condorcetResult.candidates();
    int[][] margins = condorcetResult.margins();
    int n = candidates.size();
    ...
    for (int i = 0; i < n; i++) {
      ...
      records.add(new CandidateRecord(candidates.get(i), wins, losses, ties));
    }
    return records;
  }

  /**
   * Compute the "best record" entries using the same doubled win scores as the tabulator
   * (win=2, tie=1). This ensures the report matches the actual winner-determination logic.
   */
  private List<BestRecordEntry> computeBestRecordEntries() {
    List<String> candidates = condorcetResult.candidates();
    int[][] margins = condorcetResult.margins();
    int[] wins = condorcetResult.wins();
    int n = candidates.size();
    ...
    List<BestRecordEntry> entries = new ArrayList<>();
    for (int i = 0; i < n; i++) {
      if (wins[i] == maxWins) {
        int smallestLossMargin = Integer.MAX_VALUE;
        for (int j = 0; j < n; j++) {
          if (i != j && margins[i][j] < 0) {
            smallestLossMargin = Math.min(smallestLossMargin, margins[j][i]);
          }
        }
        entries.add(new BestRecordEntry(candidates.get(i), smallestLossMargin));
      }
    }
    return entries;
  }

  // create a Condorcet-specific CSV report showing head-to-head pairwise comparison results
  private void generateCondorcetCsvReport(
      RoundTallies roundTallies,
      OutputFileIdentifiers outputFileIdentifiers) throws IOException {
    AuditableFile csvFile = createAuditableFile(outputFileIdentifiers);
    Logger.info("Generating Condorcet summary spreadsheet: %s...", csvFile.getAbsolutePath());
    Footnote tieBrokenFootnote = new Footnote("*", condorcetResult.wasDecidedViaTieBreak());
    ...
    // Contest Information section (no threshold for Condorcet)
    addContestInformationRows(csvPrinter, null, null, null, tieBrokenFootnote);

    // Contest Summary section
    addContestSummaryRows(csvPrinter, roundTallies.get(1), true);

    // Head-to-Head Results section (columnar table: one row per pair)
    csvPrinter.printRecord("Head-to-Head Results");
    csvPrinter.printRecord(
        "First Candidate", "Second Candidate",
        "Head-to-head winner", "Margin of victory",
        "Voters ranking first candidate above second",
        "Voters ranking second candidate above first",
        "Voters ranking both candidates as tied",
        "Voters ranking neither candidate",
        "Voters ranking one candidate but with no recorded preference due to skipped ranks");

    for (PairResult pair : resolvePairResults()) {
      csvPrinter.printRecord(
          pair.firstCandidate(), pair.secondCandidate(), pair.pairWinner(), pair.margin(),
          pair.firstAboveSecond(), pair.secondAboveFirst(),
          pair.tied(), pair.neitherRanked(), pair.oneRankedNoPreference());
    }
    csvPrinter.println();

    // Candidate Summary section
    csvPrinter.printRecord("Candidate Summary");
    csvPrinter.printRecord("Candidate", "Head-to-Head Wins", "Head-to-Head Losses",
        "Head-to-Head Ties");
    for (CandidateRecord rec : computeCandidateRecords()) {
      csvPrinter.printRecord(rec.candidate(), rec.wins(), rec.losses(), rec.ties());
    }
    csvPrinter.println();

    // Best Record section: candidates with the best head-to-head win score
    csvPrinter.printRecord("Best Head-to-Head Record");
    csvPrinter.printRecord("Candidate(s) with best head-to-head record",
        "Margin of smallest loss");
    for (BestRecordEntry entry : computeBestRecordEntries()) {
      String marginStr = entry.smallestLossMargin() == Integer.MAX_VALUE ? "N/A"
          : Integer.toString(entry.smallestLossMargin());
      csvPrinter.printRecord(entry.candidate(), marginStr);
    }

    if (tieBrokenFootnote.isFootnoteUsed()) {
      csvPrinter.println();
      csvPrinter.print(String.format(
          "%sWinner selected by random tiebreak after tie for most wins and smallest loss.",
          tieBrokenFootnote.getSymbol()));
      csvPrinter.println();
    }
    ...
    Logger.info("Condorcet summary spreadsheet generated successfully.");
  }

  // create a Condorcet-specific JSON report showing head-to-head pairwise comparison results
  private void generateCondorcetJsonReport(
      RoundTallies roundTallies,
      OutputFileIdentifiers outputFileIdentifiers) throws IOException {
    AuditableFile jsonFile = createAuditableFile(outputFileIdentifiers);
    Logger.info("Generating Condorcet summary JSON file: %s...", jsonFile.getAbsolutePath());
    ...
    summaryData.put("numCandidates", condorcetResult.candidates().size());
    summaryData.put("totalNumBallots", totalNumberBallots);
    summaryData.put("ballotsWithNoRankings", numNoRankings.toBigInteger());

    // results section
    HashMap<String, Object> resultsData = new HashMap<>();
    resultsData.put("winner", condorcetResult.winner());
    if (condorcetResult.wasDecidedViaTieBreak()) {
      HashMap<String, Object> tiebreakData = new HashMap<>();
      tiebreakData.put("tiebreakApplied", true);
      tiebreakData.put("tiebreakMode", config.getTiebreakMode().getInternalLabel());
      tiebreakData.put("selectedCandidate", condorcetResult.winner());
      tiebreakData.put("randomSeed", config.getRandomSeed());
      tiebreakData.put("explanation", condorcetResult.tiebreakExplanation());
      tiebreakData.put("tiedCandidates", condorcetResult.tiedCandidates());
      resultsData.put("tiebreak", tiebreakData);
    }

    // pairwise array
    ArrayList<Object> pairwiseList = new ArrayList<>();
    for (PairResult pr : resolvePairResults()) {
      HashMap<String, Object> pair = new HashMap<>();
      pair.put("candidateA", pr.firstCandidate());
      pair.put("candidateB", pr.secondCandidate());
      pair.put("aAboveB", pr.firstAboveSecond());
      pair.put("bAboveA", pr.secondAboveFirst());
      pair.put("tied", pr.tied());
      pair.put("oneRankedNoPreference", pr.oneRankedNoPreference());
      pair.put("neitherRanked", pr.neitherRanked());
      pair.put("margin", pr.margin());
      pair.put("winner", pr.pairWinner().equals("Tied") ? "tied" : pr.pairWinner());
      pairwiseList.add(pair);
    }
    resultsData.put("pairwise", pairwiseList);

    // candidate summary array
    ArrayList<Object> candidateSummaryList = new ArrayList<>();
    for (CandidateRecord rec : computeCandidateRecords()) {
      HashMap<String, Object> candidateData = new HashMap<>();
      candidateData.put("candidate", rec.candidate());
      candidateData.put("wins", rec.wins());
      candidateData.put("losses", rec.losses());
      candidateData.put("ties", rec.ties());
      candidateSummaryList.add(candidateData);
    }
    resultsData.put("candidateSummary", candidateSummaryList);

    // best record array
    ArrayList<Object> bestRecordList = new ArrayList<>();
    for (BestRecordEntry entry : computeBestRecordEntries()) {
      HashMap<String, Object> entryMap = new HashMap<>();
      entryMap.put("candidate", entry.candidate());
      if (entry.smallestLossMargin() == Integer.MAX_VALUE) {
        entryMap.put("marginOfSmallestLoss", "N/A");
      } else {
        entryMap.put("marginOfSmallestLoss", entry.smallestLossMargin());
      }
      bestRecordList.add(entryMap);
    }
    resultsData.put("bestHeadToHeadRecord", bestRecordList);
    ...
    outputJson.put("results", resultsData);
  ...
  void generateContestResultFiles(
    ...
    if (condorcetResult != null) {
      generateCondorcetCsvReport(roundTallies,
              new OutputFileIdentifiers(OutputType.DETAILED_CSV));
      generateCondorcetJsonReport(roundTallies,
